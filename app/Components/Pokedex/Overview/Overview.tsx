...
export default function Overview({pokemon, data}: OverviewProps) {
    ...
    return (
        <div className="bg-gray-800 w-full h-full rounded-xl mt-5 ml-5 flex">
            {!data ? (
                <h1 className="text-white text-2xl text-center h-30 w-full justify-center content-center">
                    Search a pokemon to start
                </h1>
            ) : (
                <>
                    <div className="flex flex-col h-100 w-60 items-center">
                        ...
                        <div className="flex">
                            <span>
                                <img
                                    className="w-25"
                                    src={`/my-react-router-app/TypeSprites/${data.types[0].type.name}.png`}
                            ...
                            {data.types.length === 2 && (
                                <span>
                                    <img
                                        className="w-25"
                                        src={`/my-react-router-app/TypeSprites/${data.types[1].type.name}.png`}
                                        alt={data.types[1].type.name}
                                    />
                                </span>
                            )}
                        </div>
                        ...
                        {stats.map((stat, index) => (
                            <div key={index} className="flex bg-gray-700 text-white p-1 rounded-lg mb-2 [&>*]:text-sm ">
                                ...
                                <span className="flex-3">
                                    <ProgressBar value={stat.value} maxValue={200} />
                                </span>
                            </div>
                        ))}
                    </div>
